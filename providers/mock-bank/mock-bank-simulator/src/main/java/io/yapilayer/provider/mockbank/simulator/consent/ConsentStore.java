...
public class ConsentStore {
    ...
    public enum Status { AWAITING_AUTHORISATION, AUTHORISED, REJECTED }
    ...
    public SimConsent createAis(List<String> permissions, Instant expiresAt) {
        SimConsent consent = new SimConsent("aisc-" + UUID.randomUUID(),
                SimConsent.Type.AIS, Status.AWAITING_AUTHORISATION, List.copyOf(permissions), expiresAt);
        consents.put(consent.id(), consent);
        return consent;
    }

    public SimConsent createPis(Instant expiresAt) {
        SimConsent consent = new SimConsent("pisc-" + UUID.randomUUID(),
                SimConsent.Type.PIS, Status.AWAITING_AUTHORISATION, List.of(), expiresAt);
        consents.put(consent.id(), consent);
        return consent;
    }
    ...
    public Optional<SimConsent> decide(String id, boolean approved) {
        return Optional.ofNullable(consents.computeIfPresent(id, (k, c) ->
                new SimConsent(c.id(), c.type(),
                        approved ? Status.AUTHORISED : Status.REJECTED,
                        c.permissions(), c.expiresAt())));
    }
}
