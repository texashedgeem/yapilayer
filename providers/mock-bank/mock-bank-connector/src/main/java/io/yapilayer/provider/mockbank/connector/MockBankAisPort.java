...
final class MockBankAisPort implements AisProviderPort {
    ...
    public ConsentStatus getConsentStatus(String providerConsentId) {
        JsonNode response =
                http.getJsonNoAuth(AISP + "/account-access-consents/" + providerConsentId);
    ...
    public ProviderSession exchangeAuthorisationCode(
            String providerConsentId, String authorisationCode) {
        JsonNode response =
                http.postForm(
                        "/oauth/token",
    ...
    public List<Balance> getBalances(ProviderSession session, AccountId accountId) {
        JsonNode response =
                http.getJson(
                        AISP + "/accounts/" + accountId.value() + "/balances",
                        session.accessToken());
    ...
    public TransactionPage getTransactions(
            ProviderSession session, AccountId accountId, PageRequest page) {
        String path =
                AISP
                        + "/accounts/"
                        + accountId.value()
