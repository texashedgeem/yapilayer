...
/**
 * Payment Initiation Services port (ADR 0004). Implemented by connectors that declare the {@code
 * PIS} capability.
 *
 * <p>Flow: {@link #createPaymentConsent} → customer authorises at the bank via the returned URL →
 * platform exchanges the callback code with {@link #exchangeAuthorisationCode} → {@link
 * #submitPayment} → status polling (or provider webhooks) until terminal.
 */
public interface PisProviderPort {
    ...
    ProviderSession exchangeAuthorisationCode(
