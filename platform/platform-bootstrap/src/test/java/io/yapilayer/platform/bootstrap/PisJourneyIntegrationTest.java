...
/**
 * Milestone 5 acceptance test: complete PIS journey through the platform's
 * public API — create payment → authorise at the (simulated) bank → callback
 * submits the payment → status polling to COMPLETED — with signed webhook
 * events received and HMAC-verified by a local receiver.
 */
@Testcontainers
...
class PisJourneyIntegrationTest {
    ...
    static void properties(DynamicPropertyRegistry registry) throws Exception {
        ...
        registry.add("yapilayer.payments-callback-url",
        ...
        webhookReceiver.createContext("/hook", exchange -> {
            try (InputStream in = exchange.getRequestBody()) {
                ...
                String signature = exchange.getRequestHeaders()
                        .getFirst("X-Yapilayer-Signature");
    ...
    void fullPisJourneyWithSignedWebhooks() throws Exception {
        ...
        ResponseEntity<Map> hook = rest.postForEntity("/api/v1/webhooks",
                Map.of("url", "http://localhost:" + hookPort + "/hook",
                        "secret", WEBHOOK_SECRET),
                Map.class);
        assertThat(hook.getStatusCode()).isEqualTo(HttpStatus.CREATED);

        // 1. Create the payment
        ResponseEntity<Map> created = rest.postForEntity("/api/v1/payments",
        ...
        ResponseEntity<Map> afterFirstPoll = rest.getForEntity(
                "/api/v1/payments/" + paymentId, Map.class);
        assertThat(afterFirstPoll.getBody().get("status")).isEqualTo("AUTHORISED");
        ResponseEntity<Map> afterSecondPoll = rest.getForEntity(
                "/api/v1/payments/" + paymentId, Map.class);
        ...
        ResponseEntity<Map> terminal = rest.getForEntity(
                "/api/v1/payments/" + paymentId, Map.class);
